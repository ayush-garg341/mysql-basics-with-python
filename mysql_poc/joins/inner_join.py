...
def prepare_data(db_con):
    ...
    cursor.execute(
        ...
        VALUES
            ...
            ('Jane Smith', 'jane.smith@example.com', '555-987-6543', '456 Elm St, Othertown, USA'),
            ('Bob Johnson', 'bob.johnson@example.com', '555-555-5555', '789 Oak St, Anycity, USA');
        """
    )
...
def retrieving_related_data(conn):
    cursor = conn.cursor()
    cursor.execute(
        """
...
def combining_info(conn):
    cursor = conn.cursor()
    cursor.execute(
        """
        SELECT customers.customer_name, orders.order_date, orders.order_id, order_details.product_name, order_details.quantity
        FROM customers
...
def calculating_aggregations(conn):
    cursor = conn.cursor()
    cursor.execute(
        """
        SELECT customers.customer_name, SUM(order_details.total_price) AS total_revenue
        FROM customers
        INNER JOIN orders ON customers.customer_id = orders.customer_id
        INNER JOIN order_details ON orders.order_id = order_details.order_id
...
if __name__ == "__main__":
    ...
    prepare_data(conn)
    retrieving_related_data(conn)
    combining_info(conn)
    calculating_aggregations(conn)
