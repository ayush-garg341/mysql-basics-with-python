...
def prepare_data(db_con):
    ...
    cursor.execute(
        ...
        VALUES
            ...
            ('Michael', 'clarke.michael@example.com', '555-456-8762', 'ACTIVE' ,'809 Oak St, Arizona, USA');
        """
    )
...
def orders_with_no_customer(conn):
    cursor = conn.cursor()
    """
    It will fetch all the orders after speicifc date, whether they have customer associated or not.
    If no customer is found for an order, customer_id will be NULL
    """
    cursor.execute(
        """
        ...
        WHERE orders.order_date >= '2023-02-20';
        """
    )
    print(cursor.fetchall())


def orders_with_specific_customer_name(conn):
    cursor = conn.cursor()
    """
    It will fetch all the orders of a customer with specific name.
    If no order is found for specific customer name, no result will be return.
    Essentially turning right join into inner join
    """
    cursor.execute(
        """
        SELECT customers.customer_name, orders.order_id
        FROM customers
        RIGHT JOIN orders ON customers.customer_id = orders.customer_id
        WHERE customers.customer_name = 'Michael';
        """
    )
    print(cursor.fetchall())
...
if __name__ == "__main__":
    ...
    prepare_data(conn)
    orders_with_no_customer(conn)
    orders_with_specific_customer_name(conn)
